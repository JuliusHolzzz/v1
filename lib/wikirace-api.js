...
function resolveSupabaseServiceKeyMeta() {
  ...
  try {
    ...
    if (raw) {
      ...
      return k ? { key: k, source: "SUPABASE_SECRET_KEYS" } : null;
    }
  } catch {
    /* ignore */
  }
...
function resolveSupabaseServiceKey() {
  return resolveSupabaseServiceKeyMeta()?.key || null;
}

function jwtKeyRole(key) {
...
function createWikiRaceApp() {
  ...
  app.get("/api/health", async (_req, res) => {
    resetSupabaseClient();
    const meta = resolveSupabaseServiceKeyMeta();
    const supabase = getSupabase();
    const keyRole = meta ? jwtKeyRole(meta.key) : null;
    const out = {
      status: "ok",
      backend: "supabase",
      base44: "disabled",
      database: Boolean(supabase),
      keySource: meta?.source || null,
      keyRole,
      keyOk: keyRole === "service_role",
      env: {
    ...
    if (meta && !out.keyOk) {
      out.hint =
        "Wrong API key: use service_role (SUPABASE_SERVICE_ROLE_KEY), not anon or publishable.";
    }
  ...
  app.all("/ws-user-apps/*", (_req, res) => {
    res.status(404).json({ error: "WebSocket disabled — WikiRace uses Supabase REST" });
  });

  app.get("/api/apps/:appId/entities/User/me", (_req, res) => {
    res.status(401).json({ error: "Not authenticated with Base44 — use WikiRace profile/guest name" });
  });

  app.put("/api/apps/:appId/entities/User/me", (_req, res) => {
    res.status(401).json({ error: "Not authenticated" });
  });

  app.get("/api/apps/:appId/entities/:entity/:id", requireDb, async (req, res) => {
    const supabase = getSupabase();
    const { entity, id } = req.params;
    const table =
      entity === "GameScore"
    ...
    try {
      ...
      if (entity === "Player") {
        ...
        return res.json(compatRow(safe));
      }
      res.json(compatRow(data));
    } catch (err) {
      res.status(500).json({ error: err.message || "Server error" });
